...
int main(int argc, char ** argv) {
    ...
    sprintf(buffer, "Hello, MPI! Rank: %d Total: %d Machine: %s", rank, size,
        name);
    char temp_1[buffer_len]; /* Change this to float*/
    ...
    if (rank == 0) {
        ...
        printf("Size : %d\n",size);
            ...
            MPI_Recv(buffer, buffer_len, MPI_CHAR/* Change this to float*/, 1, MPI_ANY_TAG,
        ...
        for ( int i = 0 ; i < 10000000; i++){
            ;//do some work
            if ( (i % 1000 )== 0) 
                    MPI_Send(buffer, buffer_len, MPI_CHAR /* Change this to float*/, 0, rank, MPI_COMM_WORLD);
        }
